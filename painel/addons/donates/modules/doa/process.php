...
if (count($_POST)) {
	...
	$donate_game = $params->get('payment_game');
	...
	if ($donateBy != 'PagSeguro') {
		...
		$transactionCredentials = new PagSeguroAccountCredentials($pagSeguroEmail, $token);

		$url = $transactionRequest->register($transactionCredentials);
		if ($url){
			$sql  = "INSERT INTO {$session->loginAthenaGroup->loginDatabase}.$donateTable";
			$sql .= "(account_id, userid, email, payment_id, payment_ip, payment_type, payment, payment_code, payment_notification_code, payment_game, payment_date) ";
			$sql .= "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
			
			$sth  = $session->loginAthenaGroup->connection->getStatement($sql);
			$res  = $sth->execute(array($donateAcc, $donateUser, $donateEmail, $donateRef, $donateIp, $donateBy, $donateVal, '1', '', $donate_game));
			header('Location:'.$url.'');
