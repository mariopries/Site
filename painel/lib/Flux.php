...
require_once 'Flux/GameDatabase.php';
...
class Flux {
	...
	public static function initializeServerObjects()
	{
		foreach (self::$serversConfig->getChildrenConfigs() as $key => $config) {
			...
			$gameDatabase = new Flux_GameDatabase($config->getGameDatabase());
			...
			foreach ($config->getCharMapServers()->getChildrenConfigs() as $charMapServer) {
				...
				$athena = new Flux_Athena($charMapServer, $loginServer, $charServer, $mapServer, $gameDatabase);
