...
/**
 * Represents an brAthena Login Server.
 */
class Flux_LoginServer extends Flux_BaseServer {
	...
	public function register($username, $password, $confirmPassword, $email, $gender, $birthdate, $securityCode)
	{
		...
		$sql = "INSERT INTO {$this->loginDatabase}.login (userid, user_pass, email, sex, group_id, birthdate, permission, token, timestamp ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$sth = $this->connection->getStatement($sql);
		$res = $sth->execute(array($username, $password, $email, $gender, (int)$this->config->getGroupID(), date('Y-m-d', $birthdatestamp), 1, 0, 0 ));
