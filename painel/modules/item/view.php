...
$tableName = "{$server->gameDatabase}.items";
...
$sql  = "SELECT $col FROM {$server->gameDatabase}.items ";
$sql .= "LEFT OUTER JOIN {$server->loginDatabase}.$shopTable ON $shopTable.nameid = items.id ";
...
if ($item) {
	...
	$mobDB      = "{$server->gameDatabase}.monsters";
	$fromTables = array("{$server->gameDatabase}.mob_db", "{$server->gameDatabase}.mob_db2");
